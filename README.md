...
1. 配置好 API Key
2. 打开命令面板（`Ctrl/Cmd + P`），搜索 **"Semlink: 全量重建索引"**
...
2. Open command palette (`Ctrl/Cmd + P`), search for **"Semlink: 全量重建索引"**
...
| Command | Description |
|---------|-------------|
| `Semlink: 全量重建索引` | Re-scan all files |
| `Semlink: 启动/停止 MCP 服务` | Toggle MCP server |
| `Semlink: 查看索引进度` | Open progress panel |
| `Semlink: 暂停索引` | Pause current indexing |
| `Semlink: 恢复索引` | Resume paused indexing |
